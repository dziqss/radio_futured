<!DOCTYPE html>
<html lang="pl">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content=" Strona radia internetowego">
    <meta name="author" content="Sławomir Radzikowski | Medius.pro">
    <meta name="keywords" content=" prezenter">
    <title>RRRRRAAAADIO INTERNETOWE</title>
    <link href="https://fonts.googleapis.com/css2?family=Roboto&display=swap" rel="stylesheet">
    <link href="favicon.ico" rel="shortcut icon" type="image/x-icon">
    <link rel="stylesheet" href="../css/style.css">
</head>

<body>
    <div id="container">

        <header>
            <img src="../img/radiologo.png" alt="analog radio - logo" />
        </header>

        <nav>
            <a class="menu" href="../index.php">RADIO</a>
            <a class="menu" href="../podstrony/ramowka.php">RAMÓWKA</a>
            <a class="menu" href="../podstrony/pozdro.php">POZDRO :-)</a>
            <a class="menu" href="../podstrony/kontakt.php">KONTAKT</a>
            <a class="menu actual" href="#">PANEL PREZENTERA</a>

        </nav>

        <section>
            <article>
                <h1>Twoja muzyka w jednym miejscu</h1>
                <p>PANEL PREZENTERA :)</p>

            </article>

            <!--Testuje odczytywanie z pliku-->
            <h1>ODCZYTAJ POZDRO</h1>

            <form action="../funkcje/odczytaj.php" method="post">
                <button>ODCZYTAJ</button>
            </form>


        </section>


        <footer>
            <center>Radio internetowe</center>

        </footer>
    </div>

</body>

</html>
